...
class Booking extends React.Component {
    ...
    componentDidMount() {
        let start = this.state.start_date.format('YYYY-MM-DD');
        let end = this.state.end_date.format('YYYY-MM-DD');
    ...
    render() {
        let start = this.state.start_date.format('MM/DD/YYYY');
        let end = this.state.end_date.format('MM/DD/YYYY');
        ...
        let propsList = homesList;
                ...
                email = {<Form.Control value={this.state.post.email} onChange={this.handleChange} name="email" type="email" placeholder="Enter email"  />}
                password = {<Form.Control value={this.state.post.password} onChange={this.handleChange} type="password" name="password" placeholder="Password" />}
                address = {<Form.Control value={this.state.post.address} onChange={this.handleChange} type="address" name="address" placeholder="1234 Main St" />}
                >
                </StrapModal>
                </div>
