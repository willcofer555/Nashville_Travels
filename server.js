...
app.get("/bookings", (req, res) => {
  ...
  db.all(
    "SELECT DISTINCT Property_name FROM Bookings WHERE ($end < Start_Date OR $start >= End_Date)",
    { $start: start, $end: end },
    (err, rows) => {
      if (err) {
...
app.post("/booking/:home", (req, res) => {
  ...
  db.all(
    "INSERT INTO Bookings(Property_Name, Start_Date, End_Date, Email) VALUES ($property, $start, $end, $email),",
    { $property: property, $email: email, $start: start, $end: end },
    (err, rows) => {
      if (err) {
...
app.post("/postbookings", (req, res, next) => {
  ...
  const sql =
    "INSERT INTO Bookings (property_name, start_date, end_date, email)" +
